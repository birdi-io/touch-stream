...
const credentials = JSON.parse(fs.readFileSync(join(thisDir, '../../../google-token.json')));
...
export function genTitle(record) {
  if (!record.teamA || !record.teamB) {
    return `${record.comp}, ${getRoundPretty(record.round)} | ${config.COMPETITION_TITLE}`;
  }
  return `${teams[record.teamA].name} vs ${teams[record.teamB].name}, ${record.comp}, ${getRoundPretty(record.round)} | ${config.COMPETITION_TITLE}`;
}
