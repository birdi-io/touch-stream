...
router.get('/', asyncHandler(async (req, res) => {
  ...
  if (req.query.start && req.query.end) {
    opts.scheduledStartTime = {
      $gte: parse(req.query.start),
    };
    opts.scheduledEndTime = {
      $lte: parse(req.query.end),
    };
  }
  if (req.query.team) {
    Object.assign(opts, {
  ...
  const matches = convertBSONUUIDs(await mongo.db.collection('matches')
    .find(opts).toArray());
  res.json({
    // TODO: Combine
    count: await mongo.db.collection('matches').estimatedDocumentCount(),
...
router.post('/:id/youtube', asyncHandler(async (req, res) => {
  const match = await mongo.db.collection('matches')
  ...
  // Insert back into db
  await mongo.db.collection('matches')
...
router.post('/:matchId/stream/:streamId', asyncHandler(async (req, res) => {
  ...
  const update = await mongo.db.collection('matches')
    .updateOne({ _id: match._id }, {
      $set: {
