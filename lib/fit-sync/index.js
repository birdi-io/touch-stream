...
router.get('/', asyncHandler(async (req, res) => {
  const [localMatches, fitMatchesRaw] = await Promise.all([
    mongo.db.collection('matches').find({}).toArray(), fit.getAllMatches(),
  ]);
  ...
  await removals.map(match =>
    mongo.db.collection('matches').deleteOne({
      id: match.id,
    }));
  // Handle additions
  ...
  if (putMatches.length) await mongo.db.collection('matches').insertMany(putMatches);
  // Handle replacements
  // TODO : THIS
  ...
  if (patchMatches.length) await mongo.db.collection('matches').bulkWrite(patchMatches);
  // TODO: Consider races
